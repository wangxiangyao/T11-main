    ...
    <div class="news-stage home-box">
      <div class="news-stage-box">
        <div class="news-stage-bg">
          <img class="news-stage-bg-img" src="../assets/home-news-bg.png" alt="">
        </div>
        <div class="news-stage-content">
          ...
          <div class="box">
            <div class="list">
              <div class="item" v-for="(v, k) in news" :key="k" @click="handleToOneArticleById(v)">
                <i class="dot"></i>
                <p class="abstract">{{ v.digest }}</p>
                <span class="time">{{ fmt('yyyy-MM-dd', new Date(v.time)) }}</span>
              </div>
            </div>
          </div>
        </div>
    ...
    <div class="elegance-display home-box margin-15p">
      ...
      <div class="box">
        <div class="item">
          ...
          <p class="abstract">
            可结合实际开发哈酒开好房间阿萨德可结合实际开发哈酒开阿拉速度快就会 发几款是的尽快哈介绍客户的骄傲和实际到货好房可结合实际
          </p>
        </div>
        <div class="item">
...
export default {
  ...
  computed: {
    list() {
      let a = this.$store.state.CarouselList.length == 0 ? '../assets/bg-header.png' : this.$store.state.CarouselList
      return a
    },
    elegance() {
      return [
